...
def AnalyseNetgrowthRW(NG_populations, max_len=880, first=10, info=None,
                       fits=True):
    ...
    The measures are generally statistical measures computed over
    ensemble average.
    The ensemble is the set of neurons present in each .swc file in 'folder'.

    Params:
    ------
    folder : (str required) the folder with the 'swc','json'
                couple as saved from NetGrowth
    ...
    fits = {}
    ensemblesRW = []
    if isinstance(NG_populations, list):
        ...
        if info:
            NG_populations["info"] = info
        ensemble = SWC_ensemble.FromPopulation(NG_populations)
        ensemble.__class__ = EnsembleRW
        ensemble.characterizeRW(max_len, first)
        fits[ensemble.name] = ensemble.fit(max_len, first)
